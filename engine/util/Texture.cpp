...
#pragma endregion
	...
	//次に格納する場所のアドレスを示す
	textureHandle += descriptorSize_;
	//次に格納する場所にアドレスを移す
	srvHandle.ptr += descriptorSize_;
	...
	texture_->srvGpuHandle_ = srvHeap.Get()->GetGPUDescriptorHandleForHeapStart();
	texture_->gpuHandle_ = srvHeap.Get()->GetGPUDescriptorHandleForHeapStart().ptr + texture_->textureHandle;
...
void TextureManager::SetGraphicsDescriptorTable(UINT index, UINT descriptorSize)
{
	//SRVヒープの設定コマンド
	std::vector<ID3D12DescriptorHeap*> heaps = { srvHeap.Get() };
	RDirectX::GetInstance()->GetCommandList()->SetDescriptorHeaps(1, heaps.data());
...
Texture* TextureManager::GetTexture(UINT64 gpuHandle)
{
	for (const auto& data : texData) {
		if (data.second->gpuHandle_ == gpuHandle)return data.second.get();
	}
	return nullptr;
}

ImGuiData TextureManager::GetImGuiData()
{
	//構造体を定義
	ImGuiData imguidate;
	//cpuハンドルを保存
	imguidate.cpuHandle = srvHandle;
	//gpuハンドルを保存
	//SRVヒープの先頭ハンドルを取得(SRVを指しているはず)
	imguidate.gpuHandle = srvHeap.Get()->GetGPUDescriptorHandleForHeapStart();
	//SRVヒープの先頭にあるSRVをルートパラメータ1番に設定
	imguidate.gpuHandle.ptr += descriptorSize_;
	//textureHandleをインクリメント
	textureHandle += descriptorSize_;
	//srvハンドルをインクリメント
	srvHandle.ptr += descriptorSize_;
	//定義した構造体を返す
