...
// ルートシグネチャ
ComPtr<ID3D12RootSignature> BoardObject::sRootSignature;
// パイプランステートの生成
ComPtr<ID3D12PipelineState> BoardObject::sPipelineState;

//コマンドリストを格納する
DirectXCommon* BoardObject::sdirectX_;
//頂点データ
Vertices BoardObject::sVertices_;
...
void BoardObject::shadowIni()
{
	HRESULT result;
	sdirectX_ = DirectXCommon::GetInstance();
	ComPtr <ID3DBlob> vsBlob = nullptr; // 頂点シェーダオブジェクト
	ComPtr <ID3DBlob> psBlob = nullptr; // ピクセルシェーダオブジェクト
	ComPtr <ID3DBlob> errorBlob = nullptr; // エラーオブジェクト
	// 頂点シェーダの読み込みとコンパイル
...
#pragma endregion
	...
	result = D3D12SerializeRootSignature(&rootSignatureDesc, D3D_ROOT_SIGNATURE_VERSION_1_0,
		&rootSigBlob, &errorBlob);
	assert(SUCCEEDED(result));
	result = sdirectX_->GetDevice()->CreateRootSignature(0, rootSigBlob->GetBufferPointer(), rootSigBlob->GetBufferSize(),
	...
	// パイプランステートの生成
	result = sdirectX_->GetDevice()->CreateGraphicsPipelineState(&pipelineDesc, IID_PPV_ARGS(&sPipelineState));
	assert(SUCCEEDED(result));

	sVertices_.Ini(sdirectX_->GetDevice());
}
...
void BoardObject::shadowPreDraw()
{
	// パイプラインステートとルートシグネチャの設定コマンド
	//パイプラインに設定した内容で描画を始める
	sdirectX_->GetCommandList()->SetPipelineState(sPipelineState.Get());
	sdirectX_->GetCommandList()->SetGraphicsRootSignature(sRootSignature.Get());

	// プリミティブ形状の設定コマンド
	sdirectX_->GetCommandList()->IASetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST); // 三角形リスト
}
...
void BoardObject::ShadowDraw(WorldTransform* worldTransform, uint32_t descriptorSize)
{
	sVertices_.Draw(6, directX_->GetCommandList(), worldTransform, descriptorSize);
}
